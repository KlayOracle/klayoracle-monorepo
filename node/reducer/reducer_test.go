...
func TestPARSE(t *testing.T) {
	...
	for index, jsonResponse := range jsons {
		...
		result, err := Parse([]interface{}{jsonResponse, reducers[index]})
...
func TestReducer(t *testing.T) {
	...
	floatResult, err := Parse([]interface{}{jsons[2], reducers[2]})
	...
	floatVal, err := StringToFloat64([]interface{}{stringFloat})
	if err != nil {
		assert.Fail(t, err.Error())
	}
	assert.Equal(t, floatVal, results[2])

	uintResult, err := Float64MulByUint([]interface{}{floatVal, 1e9})
	if err != nil {
		assert.Fail(t, err.Error())
	}
